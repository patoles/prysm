		...
		initShaders: {
			value: function initShaders() {
				...
				this.waveList.forEach(function (item, key) {
					_this.shaderProgram.wave[key] = {};
					_this.shaderProgram.wave[key].center = _this.ctx.getUniformLocation(_this.shaderProgram, "wave[" + key + "].center");
					_this.shaderProgram.wave[key].time = _this.ctx.getUniformLocation(_this.shaderProgram, "wave[" + key + "].time");
					_this.shaderProgram.wave[key].shockParams = _this.ctx.getUniformLocation(_this.shaderProgram, "wave[" + key + "].shockParams");
					_this.shaderProgram.wave[key].hasShock = _this.ctx.getUniformLocation(_this.shaderProgram, "wave[" + key + "].hasShock");
				});
			}
		},
		handleLoadedTexture: {
...
var Canvas3D = (function (_CanvasWebgl) {
	...
	_createClass(Canvas3D, {
		...
		updateTexture: {
			value: function updateTexture(object, url) {
				var _this = this;
		...
		render: {
			value: function render() {
				_get(Object.getPrototypeOf(Canvas3D.prototype), "render", this).call(this);
			}
		},
		draw: {
			value: function draw() {
				...
				this.ctx.clear(this.ctx.COLOR_BUFFER_BIT | this.ctx.DEPTH_BUFFER_BIT);
				this.perspectiveMatrix = this.glUtils.makePerspective(perspectiveValue, this.screenWidth / this.screenHeight, 0.1, 100);
				this.glUtils.loadIdentity();
				this.meshList.forEach(function (item, key) {
					_this.glUtils.mvPushMatrix();
					_this.glUtils.mvRotateMultiple(_this.xRotation, [1, 0, 0], _this.yRotation, [0, 1, 0]);
					_this.glUtils.mvTranslate(item.translation);
					if (item.scale) _this.glUtils.mvScale([item.scale[0], item.scale[1], item.scale[2]]);
					if (item.objName === "sphere" || item.objName === "cube") _this.glUtils.mvRotateMultiple(_this.ballRotation, [1, 0, 0], _this.ballRotation, [0, 1, 0]);
					_this.drawObject(_this.meshes[item.name], item.color || [0.5, 1, 1, 1]);
					_this.glUtils.mvPopMatrix();
				});
				this.transform();
			}
		},
