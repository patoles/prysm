...
class GlUtils{
	...
	mvTranslate(v){
		this.multMatrix(Matrix.Translation(new Vector([v[0], v[1], v[2]])).ensure4x4());
	}
	...
	mvRotateMultiple(angleA, vA, angleB, vB){
		...
		var mA = Matrix.Rotation(inRadians, new Vector([vA[0], vA[1], vA[2]])).ensure4x4();
		inRadians = angleB * Math.PI / 180.0;
		var mB = Matrix.Rotation(inRadians, new Vector([vB[0], vB[1], vB[2]])).ensure4x4();
		var m = mA.x(mB);
		this.multMatrix(m);
	}
	mvScale(size){
